...
use revm::database::{BundleState, WrapDatabaseRef};
    ...
    C::Primitives: FromInput + ValidateBlockPostExecution,
{
    pub fn execute(
        ...
        let num_states = executor_outcome.state().state.len() as u64;
        // if executor_outcome.state().state.len() != 80 {
    ...
    pub fn execute_new(
        &self,
        mut input: ClientExecutorInput<C::Primitives>,
    ) -> Result<BundleState, ClientError> {
        // Initialize the witnessed database with verified storage proofs.
        let db = profile_report!(INIT_WITNESS_DB, {
            let trie_db = input.witness_db().unwrap();
            WrapDatabaseRef(trie_db)
        ...
        Ok(executor_outcome.state().clone())
